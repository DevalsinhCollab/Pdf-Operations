...
// Function to handle the complete process: PDF -> Images -> PDF -> Base64
async function convertPdfToImagesAndBack(pdfFilePath) {
    try {
        ...
        const base64String = pdfToBase64(pdfBytes);
        console.log('Base64 string of the recreated PDF:', base64String);

        // Save the Base64 string to a text file
        const base64OutputPath = path.join(__dirname, 'output', 'pdf_base64.txt');
        saveBase64ToFile(base64String, base64OutputPath);
...
convertPdfToImagesAndBack(pdfFilePath);